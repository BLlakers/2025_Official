...
/*public void resetAlgaePidThroughBore(){
    ...
    public Command AlgaePIDUp(){
        return resetAlgaePIDCmd().andThen(runEnd(() -> AlgaePID(posUpreg), this::AlgaeStop).onlyWhile(() -> !CheckAlgaePID()));
    }

    public Command AlgaePIDDown(){
        return resetAlgaePIDCmd().andThen(runEnd(() -> AlgaePID(posDownreg), this::AlgaeStop).onlyWhile(() ->!CheckAlgaePID()));
    }
    public Command AlgaePIDMiddle(){
        return resetAlgaePIDCmd().andThen(runEnd(() -> AlgaePID(posMiddlereg), this::AlgaeStop).onlyWhile(() ->!CheckAlgaePID()));
    }
    ...
    // public Command AlgaePIDThroughBore(){
    //     if(AlgaeIR() <= 100){
    ...
    //     return resetAlgaePIDThroughBoreCmd().andThen(runEnd(() -> AlgaeThroughBorePID(-0.0135), this::AlgaeStop).onlyWhile(() ->!CheckAlgaePIDThroughBore()));
    // }
